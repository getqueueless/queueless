...
-- Three named, narrow exceptions, all recorded/administrative DATA rather than a substitute
-- for now(), and none does any time-window or rate-limit math with the date it takes:
...
--                                                        not a patient-facing abuse surface
select is_empty(
  $$
  ...
  $$,
  'no function in schema public accepts a timestamptz/timestamp/date argument, except the three named, narrow exceptions'
);
