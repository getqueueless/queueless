-- P0, found while building the rest of this migration: 0066's default-privilege lockdown
-- ("alter default privileges for role postgres in schema public revoke ... from anon,
-- authenticated") never touched PUBLIC itself. Every role, including queueless_api and anon, is
-- implicitly a member of PUBLIC -- Postgres's own built-in default is EXECUTE on every new
-- function granted TO PUBLIC, unless that's revoked too. A function that drops and recreates an
-- existing signature (start_paid_booking/start_paid_appointment below both do, since their
-- argument list changed) loses whatever explicit grants it had and starts over on that default
-- -- caught by 105_queueless_api_least_privilege.test.sql unexpectedly seeing queueless_api with
-- execute on both, and confirmed empirically that anon had it too. Closing it at the source, the
-- same way 0066 meant to.
alter default privileges for role postgres in schema public revoke execute on functions from public;

-- Patient-requested priority (Yash). Today only staff set a lane (verify_priority), so the app
-- has no way to know a patient is pregnant/elderly/emergency before they're physically at the
-- desk. This lets a patient flag it when booking; staff still verifies before it actually
-- changes queue order -- except 'senior', which the database can already confirm on its own from
-- date_of_birth, so that one applies immediately (still recorded, so staff sees it for an ID
-- check, same as the manual ones).

-- requested_lane/requested_note live on BOTH tokens and appointments -- appointments have no
-- lane/priority_at of their own (they're slot-based, not queue-based), so a request made at
-- start_paid_appointment time is only informational until check_in actually mints a token; a
-- request made at start_paid_booking time (no appointment involved) applies directly.
alter table public.tokens add column requested_lane public.lane;
alter table public.tokens add column requested_note text;
alter table public.tokens add constraint tokens_requested_lane_valid
  check (requested_lane is null or requested_lane in ('senior', 'pregnant', 'emergency'));
alter table public.tokens add constraint tokens_requested_note_len
  check (requested_note is null or char_length(requested_note) <= 80);

alter table public.appointments add column requested_lane public.lane;
alter table public.appointments add column requested_note text;
alter table public.appointments add constraint appointments_requested_lane_valid
  check (requested_lane is null or requested_lane in ('senior', 'pregnant', 'emergency'));
alter table public.appointments add constraint appointments_requested_note_len
  check (requested_note is null or char_length(requested_note) <= 80);

-- profiles.priority_status (0002) only ever allowed senior/pregnant -- 'emergency' is now a
-- real verifiable status too.
alter table public.profiles drop constraint profiles_priority_status_valid;
alter table public.profiles add constraint profiles_priority_status_valid
  check (priority_status is null or priority_status in ('senior', 'pregnant', 'emergency'));

-- The one thing the database can verify on its own, no staff judgment call needed.
create function private.patient_is_senior(p_patient uuid)
returns boolean
language sql
stable
security definer
set search_path = ''
as $$
  select date_of_birth is not null and date_of_birth <= (current_date - interval '60 years')::date
  from public.profiles where id = p_patient;
$$;
revoke execute on function private.patient_is_senior(uuid) from public, anon, authenticated;

-- start_paid_booking (0069) redefined with 2 new trailing DEFAULT params. CREATE OR REPLACE does
-- NOT replace a shorter-arity function -- confirmed the hard way: it silently created a second
-- overload alongside the old 1-arg one, making every existing 1-arg call site ambiguous
-- ("function is not unique"). The old signature is dropped explicitly first, so there is only
-- ever one start_paid_booking; old callers keep working via p_requested_lane/p_note's defaults.
drop function public.start_paid_booking(uuid);
...
begin
  ...
  insert into public.tokens (
    ...
    status, patient_id, doctor_id, fee_inr, hold_expires_at, requested_lane, requested_note
  ) values (
...
-- Dropping the old signature above wipes ITS grants too -- the new one starts from scratch, and
-- (found the hard way, via a queueless_api least-privilege test catching it) 0066's default-
-- privilege lockdown only covers anon/authenticated, never PUBLIC itself, so a function with no
-- explicit grant of its own is executable by literally everyone via the implicit PUBLIC
-- membership every role has -- including queueless_api and, worse, anon. Restated explicitly.
...
-- start_paid_appointment (0069) redefined the same way -- old 1-arg signature dropped first,
-- same reasoning as start_paid_booking above. Appointments have no lane/priority_at of their
-- own -- 'senior' still auto-applies (recorded now, same as a manual request, but there's
-- nothing to "elevate" yet); the real effect (lane + pulled-forward priority_at) lands on
-- whatever token check_in eventually mints from this appointment, below.
drop function public.start_paid_appointment(uuid);
...
revoke execute on function public.start_paid_appointment(uuid, public.lane, text) from public, anon;
grant execute on function public.start_paid_appointment(uuid, public.lane, text) to authenticated;

-- check_in and verify_priority below keep their original argument lists, so `create or replace`
-- genuinely replaces them in place and their existing grants carry over untouched -- no new
-- revoke/grant needed for either.

-- check_in (0014) redefined to carry an appointment's priority request onto the token it mints
-- -- otherwise a patient who requested priority at booking time would lose it the moment they
-- check in, and staff would never see it. 'senior' applies immediately (lane + pulled-forward
-- priority_at, same rule tokens get at start_paid_booking); pregnant/emergency stay pending,
-- same as always, waiting on verify_priority. mint_token itself is untouched (still called with
-- lane='appointment', unaffected callers elsewhere keep working exactly as before) -- this is a
-- follow-up update on the row it returns.
  ...
  if v_appt.requested_lane is not null then
    if v_appt.requested_lane = 'senior' then
      ...
      update public.tokens
        set requested_lane = v_appt.requested_lane, requested_note = v_appt.requested_note,
...
-- verify_priority (0015) redefined: adds 'emergency' as a verifiable status, and 'normal' as an
-- explicit reject path (clears requested_lane/requested_note, never touches lane/priority_at --
-- a pending request never elevated either in the first place).
...
begin
  ...
  if p_status = 'normal' then
    ...
    update public.tokens set requested_lane = null, requested_note = null
      where id = p_token
      returning * into v_token;
    return v_token;
  end if;
