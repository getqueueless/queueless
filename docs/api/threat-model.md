...
| T10 | This API's responses framed inside a malicious page (clickjacking) | Spoofing |
...
- **T5** — `CORSMiddleware` is configured with an explicit allowlist from `settings.cors_allow_origins` (env-driven), never `"*"`, and the origin is never dynamically reflected. Re-reviewed this session: no `allow_origin_regex`, no wildcard, and `allow_credentials=True` paired with an explicit list (not `"*"`, which Starlette's own `CORSMiddleware` would refuse to serve credentialed responses for anyway) -- nothing changed, re-confirmed rather than assumed still true.
...
- **T10** — `X-Frame-Options: DENY` plus, found missing and fixed this session, an explicit `frame-ancestors 'none'` on every response's CSP (`app/middleware.py`, both the default policy and the relaxed `/docs` one). `default-src 'none'` does **not** cover `frame-ancestors` — that directive has its own default (unrestricted) when omitted, a real gap this session's security pass caught rather than assumed covered.
...
- **Dependency supply chain: clean as of 2026-09-26, not a standing guarantee.** `pip-audit`
  against the real installed environment (`uv run --with pip-audit pip-audit`, confirmed scanning
  the actual project venv, not an empty one) found no known vulnerabilities in any of `apps/api`'s
  dependencies. This is a point-in-time result, not something CI re-checks automatically yet — a
  scheduled `pip-audit` run (or Dependabot-equivalent) is the real fix if this matters ongoing,
  not re-running it by hand each session.
